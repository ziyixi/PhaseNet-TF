...
class GenSgram(Spectrogram):
    ...
    def __init__(self, n_fft: int = 256, hop_length: int = 1, freqmin: float = 0, freqmax: float = 10, dt_s: float = 0.025, height: int = 64, width: int = 4800, max_clamp: int = 3000):
        """
        Args:
        ...
        super().__init__(n_fft=n_fft, hop_length=hop_length,
                         power=None, window_fn=torch.hann_window)
        self.n_fft = n_fft
