...
from .base_inference import (BatchToWaveformTransform, PadZeroTransform,
                             SlidingWindowNormalizeTransform,
                             WaveformToBatchTransform)


def single_inference(model: LightningModule, data: torch.tensor, extract_phases: List[str], model_phases: List[str], extract_phase_sensitivity: List[float], dt_s: float, window_length_in_npts: int, hop_length_in_npts: int, sensitive_distances_in_seconds: float) -> Dict[str, Dict[str, List]]:
    ...
    pad_zero_transform = PadZeroTransform(
    ...
    phase_sensitivity.update(
        {phase: sensitivity for phase, sensitivity in zip(extract_phases, extract_phase_sensitivity)})
    ...
    # extract peaks
    peaks = extract_peaks(predict_batch=output,
                          phases=model_phases,
    ...
    for iphase, phase in enumerate(model_phases):
        if phase in extract_phases:
            ...
            output_keep_only_extract_phases[phase] = output[0][iphase +
                                                               1].numpy().tolist()
