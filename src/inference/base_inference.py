    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if npts % self.hop_length_in_npts != 0:
            x = torch.cat((x, torch.zeros(
                (3, self.hop_length_in_npts - npts % self.hop_length_in_npts))), dim=1)
        return x
...
class SlidingWindowNormalizeTransform:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        means, stds = [], []
        for i in range(0, npts - self.window_length_in_npts + 1, self.hop_length_in_npts):
        ...
        means = torch.nn.functional.interpolate(
            means.unsqueeze(0), size=npts, mode="linear", align_corners=False).squeeze(0)
        stds = torch.nn.functional.interpolate(
            stds.unsqueeze(0), size=npts, mode="linear", align_corners=False).squeeze(0)
...
class BatchToWaveformTransform:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        npts = number_of_windows*self.hop_length_in_npts + \
            self.window_length_in_npts-self.hop_length_in_npts
        res = torch.zeros((channel, npts))
        for i in range(number_of_windows):
            # for the overlap part, we take the max of the two/three/... windows
            res[:, i*self.hop_length_in_npts:i*self.hop_length_in_npts+self.window_length_in_npts] = torch.max(
                res[:, i*self.hop_length_in_npts:i*self.hop_length_in_npts+self.window_length_in_npts], x[i, :, :])
        return res
